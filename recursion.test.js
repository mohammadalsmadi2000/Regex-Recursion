...
function division(number, dividedBy) {
    if (dividedBy == 0 || number == 0 || number < dividedBy) return 0;
...
//it needed to search at long time to find a solution
function permutations(n, k) {
    ...
    generatePermutations(nums, [], arr);
    return arr;
};
function generatePermutations(nums, permutation, arr) {

    if (nums.length === 0) {
        arr.push(permutation.join(""));
        return;
    }
    for (let i = 0; i < nums.length; i++) {
        let newNums = nums.slice(0, i).concat(nums.slice(i + 1));
        let newPermutation = permutation.concat([nums[i]]);
        generatePermutations(newNums, newPermutation, arr);
    }
}
